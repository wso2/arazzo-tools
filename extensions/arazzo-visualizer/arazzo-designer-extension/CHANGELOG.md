...
- **Try with curl**: Added a standard execution mode for running workflows directly in the terminal
  - Workflow-level CodeLens and visualizer webview button to execute workflows via curl
...
- **Automatic TLS Error Handling**: Added smart detection for certificate validation failures
  ...
  - Manual toggle available in VS Code settings under `Arazzo Visualizer > Disable TLSCertification Validation`
- **Workflow Input Configuration**: Added a dedicated panel for managing workflow input parameters
  - Inline validation for required fields
  - Visual feedback and hover descriptions for configuration errors
