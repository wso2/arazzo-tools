...
- **Passing (✅):** `01`, `03` (run `mainFlow`), `06` (run `consumer`) → the workflow completes.
- **Failing on purpose (❌):** `02`, `04`, `05` → the run ends in an error with the message shown
  above. That error **is** the expected outcome — it's the gate / cycle detection doing its job.
